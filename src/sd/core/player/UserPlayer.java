...
public class UserPlayer extends UnicastRemoteObject implements
	 ...
	 */
	public void updateStatus(final List<Partecipant> partecipants, final GameBoard gameBoard, final String ipCurrentPartecipant, final int currentDie) throws RemoteException {
		new Thread() {
			public void run() {
				try {
					SwingUtilities.invokeAndWait(new Runnable() {
	...
	public void updateNext(List<Partecipant> partecipants, GameBoard gameBoard, String ipCurrentPartecipant, int currentDie) {
			try {
				...
				UserPlayerInterface nextInTurn = (UserPlayerInterface) Naming.lookup("rmi://"+ nextInTurnId + "/RMIGameClient");
				nextInTurn.updateStatus(partecipants, gameBoard, ipCurrentPartecipant, currentDie);
